...
const mapCoinGeckoToCryptoData = (apiData) => ({
    id: apiData.id,
    name: apiData.name,
...
export const fetchIdrRate = async () => {
...
export const fetchTrendingCoins = async () => {
    ...
    const trendingIds = trendingData.coins.map((c) => c.item.id).slice(0, 7).join(',');
    if (!trendingIds)
        return [];
...
export const fetchTop500Coins = async () => {
...
export const fetchExchangeTickers = async (coinId) => {
