...
const ReactionPicker = ({ onSelect, onClose }: { onSelect: (emoji: string) => void; onClose: () => void; }) => (
    ...
    <div className="absolute bg-gray-900/80 backdrop-blur-md border border-white/10 rounded-lg p-1 flex items-center gap-1 z-20 shadow-lg animate-fade-in-fast whitespace-nowrap"> {/* Added whitespace-nowrap */}
        {EMOJIS.map(emoji => (
...
// Display Existing Reactions (No changes needed)
...
// Delete Button Component (No changes needed)
const DeleteButton: React.FC<{ onClick: (e: React.MouseEvent) => void }> = ({ onClick }) => (
    // ... (kode DeleteButton tetap sama) ...
...
// React Button Component (opens picker) (No changes needed)
const ReactButton: React.FC<{ onClick: (e: React.MouseEvent) => void }> = ({ onClick }) => (
    // ... (kode ReactButton tetap sama) ...
        ...
        <div className="my-2 animate-fade-in-up relative">
            ...
            >
                <div className="relative w-full sm:w-4/5 md:w-3/5 mx-auto">
                    ...
                    {/* Action Buttons Container - Relative for positioning picker */}
                    <div className={`absolute top-0 right-1 z-10 flex items-center bg-gray-900 border border-white/10 rounded-full px-1 shadow-lg transition-opacity duration-200 ${showActions ? 'opacity-100 animate-fade-in-fast' : 'opacity-0 pointer-events-none'}`}>
                        ...
                        {/* Picker - Positioned below the action buttons */}
                        {showPicker && (
                            <ReactionPicker
                    ...
                    {/* Content */}
                    <div className={`block p-3 bg-gray-800/50 rounded-lg transition-colors mt-6 ${isActive ? 'bg-gray-800/60' : ''}`}> {/* Added mt-6 for spacing */}
...
const UserMessage: React.FC<{
   // ... (props tetap sama) ...
    ...
    return (
        // Added 'relative' to the main container for absolute positioning context
        <div className={`my-1 py-1 flex relative ${isCurrentUser ? 'justify-end' : 'justify-start'}`}>
            ...
            <div className={`flex items-start gap-2 ${isCurrentUser ? 'flex-row-reverse' : 'flex-row'}`}>
                 ...
                 {/* Action Buttons & Picker Container - Positioned absolutely relative to the main div */}
                 {/* Placed OUTSIDE the message content div */}
                 <div className={`absolute top-0 z-10 flex items-center bg-gray-900 border border-white/10 rounded-full px-1 shadow-lg transition-opacity duration-200 ${showActions ? 'opacity-100 animate-fade-in-fast' : 'opacity-0 pointer-events-none'} ${isCurrentUser ? 'right-full mr-1' : 'left-full ml-1'}`}>
                    ...
                    {/* Adjusted position: top-0 left-full/right-full */}
                    {showPicker && (
                         <ReactionPicker
                ...
                {/* Removed 'relative' from here as context is now the parent */}
...
const ForumPage: React.FC<ForumPageProps> = ({
    ...
    const handleMessageClick = (messageId: string) => { /* ... code ... */ setActiveMessageId(currentId => { if (currentId === messageId) { const newShowActions = !showActions; setShowActions(newShowActions); setShowPickerForMsgId(null); return newShowActions ? currentId : null; } else { setShowActions(true); setShowPickerForMsgId(null); return messageId; } }); };
    const handleReactButtonClick = (e: React.MouseEvent, messageId: string) => { /* ... code ... */ e.stopPropagation(); setShowPickerForMsgId(currentPickerId => currentPickerId === messageId ? null : messageId ); if (activeMessageId === messageId && !showActions) { setShowActions(true); } else if (activeMessageId !== messageId) { setActiveMessageId(messageId); setShowActions(true); } };
    const handleChatAreaClick = (e: React.MouseEvent<HTMLDivElement>) => { /* ... code ... */ if (e.target === e.currentTarget && (showActions || activeMessageId || showPickerForMsgId)) { setShowActions(false); setActiveMessageId(null); setShowPickerForMsgId(null); } };
    const safeMessages = Array.isArray(messages) ? messages : [];
    const sortedMessages = useMemo(() => { /* ... code ... */ return [...safeMessages].sort((a, b) => { const timeA = isNewsArticle(a) ? (a.published_on * 1000) : (isChatMessage(a) ? a.timestamp : 0); const timeB = isNewsArticle(b) ? (b.published_on * 1000) : (isChatMessage(b) ? b.timestamp : 0); if (!timeA && !timeB) return 0; if (!timeA) return 1; if (!timeB) return -1; return timeA - timeB; }); }, [safeMessages]);
    useEffect(() => { /* ... code ... */ if(activeMessageId === null && chatEndRef.current) { chatEndRef.current.scrollIntoView({ behavior: 'smooth', block: 'end' }); } }, [sortedMessages, activeMessageId]);
    const handleSendMessageSubmit = (e: React.FormEvent<HTMLFormElement>) => { /* ... code ... */ e.preventDefault(); const currentMessageText = newMessage.trim(); const currentAttachment = attachment; if ((!currentMessageText && !currentAttachment) || !username) { return; } const messageData: Partial<ChatMessage> & { type: 'user'; sender: string; timestamp: number } = { type: 'user', sender: username, timestamp: Date.now(), reactions: {}, ...(currentMessageText && { text: currentMessageText }), ...(currentAttachment && { fileURL: currentAttachment.dataUrl, fileName: currentAttachment.name }), }; onSendMessage(messageData as ChatMessage); setNewMessage(''); setAttachment(null); if(fileInputRef.current) fileInputRef.current.value = ""; };
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => { /* ... code ... */ const file = e.target.files?.[0]; if (file && file.type.startsWith('image/')) { const reader = new FileReader(); reader.onloadend = () => { if (reader.result) { setAttachment({ dataUrl: reader.result as string, name: file.name }); } }; reader.readAsDataURL(file); } else { setAttachment(null); } };
                     ...
                     {sortedMessages.length === 0 ? (
                        /* ... (kode pesan kosong) ... */
                         ...
                         sortedMessages.map((item, index) => {
                            {/* ... (logika map pesan tetap sama, memanggil UserMessage/NewsMessage) ... */}
