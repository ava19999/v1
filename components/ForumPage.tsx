...
const MessageContainer: React.FC<{ className?: string, children: React.ReactNode }> = ({ children, className }) => (
    <div className={`relative group ${className}`}>{children}</div>
);

const ReactionPicker = ({ onSelect }: { onSelect: (emoji: string) => void; }) => (
    <div className="absolute top-0 right-0 opacity-0 group-hover:opacity-100 transition-opacity duration-200 bg-gray-900/80 backdrop-blur-md border border-white/10 rounded-full p-1 flex items-center gap-1 z-10">
        {EMOJIS.map(emoji => (
            <button key={emoji} onClick={() => onSelect(emoji)} className="text-xl hover:scale-125 transition-transform p-1 rounded-full hover:bg-white/10">
                {emoji}
            </button>
        ))}
    </div>
...
const NewsMessage: React.FC<{ article: NewsArticle; username: string; onReact: (messageId: string, emoji: string) => void; }> = ({ article, username, onReact }) => (
    <MessageContainer className="my-2 animate-fade-in-up">
        ...
        <div className="relative w-full sm:w-4/5 md:w-3/5 mx-auto">
            ...
            <Reactions message={article} username={username} onReact={(emoji: string) => onReact(article.id, emoji)} />
            <ReactionPicker onSelect={(emoji: string) => onReact(article.id, emoji)} />
        </div>
    </MessageContainer>
);
...
const UserMessage: React.FC<{ message: ChatMessage; userProfile: User | null; onReact: (messageId: string, emoji: string) => void; }> = ({ message, userProfile, onReact }) => {
    ...
    return (
        <MessageContainer className={`my-1 animate-fade-in-up py-1 flex ${isCurrentUser ? 'justify-end' : 'justify-start'}`}>
                ...
                <div className={`flex items-start gap-3 ${isCurrentUser ? 'flex-row-reverse' : ''}`}>
                    <div className="flex-1 overflow-hidden">
                        <div className={`flex items-center gap-2 flex-wrap ${isCurrentUser ? 'flex-row-reverse' : ''}`}>
                            ...
                            <UserTag sender={message.sender} userCreationDate={userProfile?.createdAt ?? null} />
                        </div>
                        ...
                        <Reactions message={message} username={currentUsername} onReact={(emoji: string) => onReact(message.id, emoji)} />
                    </div>
                </div>
                {!isCurrentUser && (
                    <div className="relative h-full">
                        <ReactionPicker onSelect={(emoji: string) => onReact(message.id, emoji)} />
                    </div>
                )}
            </div>
        </MessageContainer>
    ...
    return (
        <div className="container mx-auto px-2 sm:px-4 py-3 animate-fade-in flex flex-col flex-grow h-[calc(100vh-56px)]">
            ...
            <div className="bg-gray-900 border border-white/10 rounded-xl flex flex-col flex-grow overflow-hidden">
                <div className="p-2 md:p-4 flex-grow overflow-y-auto space-y-1">
                     {sortedMessages.length === 0 ? (
                        ...
                        sortedMessages.map((item, index) => {
                           ...
                           if (isNewsArticle(item)) return <NewsMessage key={item.id + index} article={item} username={username} onReact={onReact} />;
                           if (isChatMessage(item)) {
                               if (item.type === 'system') return <SystemMessage key={item.id} message={item} />;
                               return <UserMessage key={item.id} message={item} userProfile={userProfile} onReact={onReact} />;
                           }
                           return null;
                        })
                    )}
