    ...
    <div className="my-1 animate-fade-in-up">
      <div className="w-full sm:w-4/5 md:w-3/5 mx-auto relative">
        <div
          ...
          className={`block p-2 bg-gray-800/50 rounded-lg transition-colors cursor-pointer ${
        ...
        >
          <div className="flex items-start space-x-2">
            <img
              ...
              alt={article.title}
              className="w-16 h-12 object-cover rounded-md flex-shrink-0 bg-gray-700 cursor-zoom-in"
              loading="lazy"
              onClick={(e) => {
                e.stopPropagation();
        ...
        <div className="relative mt-0.5 px-0.5 flex justify-between items-start">
          ...
          <div className="relative flex-shrink-0">
            <div className={`flex items-center gap-0.5 transition-opacity duration-200 ${showActions ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
    ...
    <div className={`my-0.5 flex relative ${isCurrentUser ? 'justify-end' : 'justify-start'}`}>
      <div className={`max-w-xs sm:max-w-sm md:max-w-md ${isCurrentUser ? 'ml-auto' : ''}`}>
        ...
        >
          {message.fileURL && (
            <img
              ...
              className="rounded-md max-h-32 mb-0.5 cursor-zoom-in"
              onClick={(e) => {
                e.stopPropagation();
        ...
        <div className="relative mt-0.5 px-0.5 flex justify-between items-start">
          ...
          <div className={`relative flex-shrink-0 ${isCurrentUser ? 'ml-0.5' : 'mr-0.5'}`}>
            <div className={`flex items-center gap-0.5 transition-opacity duration-200 ${showActions ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
              <ReactButton onClick={onReactButtonClick} />
              {canDelete && <DeleteButton onClick={(e) => { e.stopPropagation(); onDeleteClick(); }} />}
            </div>
...
const AnnouncementMessage: React.FC<{ message: ChatMessage }> = ({ message }) => {
  ...
  return (
    <div className={`bg-gray-800/50 border-l-4 ${borderColor} rounded-r-lg p-3 my-2 animate-fade-in-up`}>
      <div className="flex items-start gap-3">
        <div className={`flex-shrink-0 ${titleColor}`}>{icon}</div>
        <div className="flex-1">
          {title && <h3 className={`text-base font-bold ${titleColor} mb-0.5`}>{title}</h3>}
  ...
  return (
    <div className="flex flex-col flex-grow h-full">
              ...
              {attachment && (
                <div className="relative inline-block">
                  ...
                  <button
                    ...
                    className="absolute -top-1 -right-1 bg-red-500 text-white rounded-full h-4 w-4 flex items-center justify-center text-[10px] font-bold hover:bg-red-700 transition-colors"
      ...
      <style>{`
        ...
        .custom-scrollbar::-webkit-scrollbar-thumb { background: rgba(255, 255, 255, 0.2); border-radius: 1.5px; }
