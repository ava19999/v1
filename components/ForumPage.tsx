    ...
    return (
        <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
            {hasReactions && Object.entries(reactions).map(([emoji, users]) => {
                ...
                if (!userList || userList.length === 0) return null; // Add check for empty userList
                return (
                    <button
                        key={emoji}
...
const MessageWrapper: React.FC<{
    messageId: string;
    messageData: NewsArticle | ChatMessage; // Pass the full data object
    username: string;
    onReact: (messageId: string, emoji: string) => void;
    children: React.ReactNode;
    className?: string;
    reactionContainerClassName?: string;
}> = ({ messageId, messageData, username, onReact, children, className, reactionContainerClassName }) => {
    ...
    return (
        <div className={`relative ${className}`}>
            ...
            <div ref={pickerRef} className={`relative ${reactionContainerClassName || ''}`}>
                 {/* Pass messageData directly */}
                <Reactions
                    message={messageData} // Pass the full message data object
                    username={username}
...
const NewsMessage: React.FC<{ article: NewsArticle; username: string; onReact: (messageId: string, emoji: string) => void; }> = ({ article, username, onReact }) => (
    <MessageWrapper
        ...
        messageData={article} // Pass article data
        username={username}
        onReact={onReact}
...
const UserMessage: React.FC<{ message: ChatMessage; userProfile: User | null; onReact: (messageId: string, emoji: string) => void; }> = ({ message, userProfile, onReact }) => {
    ...
    return (
        <MessageWrapper
            ...
            messageData={message} // Pass message data
                ...
                <div className={`flex items-start gap-3 ${isCurrentUser ? 'flex-row-reverse' : ''}`}>
                    <div className="flex-1 overflow-hidden">
                        <div className={`flex items-center gap-2 flex-wrap ${isCurrentUser ? 'flex-row-reverse' : ''}`}>
                            ...
                            {/* Teruskan creationDate yang sudah ditentukan */}
...
const ForumPage: React.FC<ForumPageProps> = ({
    room, messages = [], userProfile, onSendMessage, onLeaveRoom, onReact // Default messages ke array kosong
}) => {
    ...
    // Pastikan messages selalu array sebelum di-sort
    const safeMessages = Array.isArray(messages) ? messages : [];

     // Sorting messages (sama seperti di App.tsx)
    ...
    }, [safeMessages]); // Gunakan safeMessages
    ...
    const handleSendMessage = (e: React.FormEvent<HTMLFormElement>) => { /* Kirim pesan */
        e.preventDefault(); if ((!newMessage.trim() && !attachment) || !username) return;
                ...
                <div className="p-2 md:p-4 flex-grow overflow-y-auto space-y-1 custom-scrollbar"> {/* Add custom-scrollbar */}
                     {sortedMessages.length === 0 ? (
                        ...
                        sortedMessages.map((item, index) => {
                           // Gunakan type guard untuk menentukan komponen yang dirender
                           if (isChatMessage(item)) {
                               if (item.type === 'system') {
                                   // Render AnnouncementMessage khusus di room aturan
                                   if (room.id === 'pengumuman-aturan') {
                                        return <AnnouncementMessage key={item.id || `sys-${index}`} message={item} />;
                                   }
                               ...
                               return <NewsMessage key={item.id || `news-${index}`} article={item} username={username} onReact={onReact} />;
                           }
                           return null; // Handle unexpected item types
                        })
                    )}
                    <div ref={chatEndRef} />
                </div>
