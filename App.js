...
const Particles = () => (_jsxs("div", { className: "particles", children: [_jsx("div", { className: "particle", style: { width: '3px', height: '3px', left: '10%', animationDelay: '-1s' } }), _jsx("div", { className: "particle", style: { width: '2px', height: '2px', left: '25%', animationDelay: '-5s' } }), _jsx("div", { className: "particle", style: { width: '4px', height: '4px', left: '50%', animationDelay: '-3s' } }), _jsx("div", { className: "particle", style: { width: '3px', height: '3px', left: '70%', animationDelay: '-8s' } }), _jsx("div", { className: "particle", style: { width: '2px', height: '2px', left: '85%', animationDelay: '-2s' } }), _jsx("div", { className: "particle", style: { width: '4px', height: '4px', left: '40%', animationDelay: '-12s' } }), _jsx("div", { className: "particle", style: { width: '3px', height: '3px', left: '90%', animationDelay: '-15s' } })] }));
const App = () => {
    ...
    const [joinedRoomIds, setJoinedRoomIds] = useState(() => new Set(['berita-kripto', 'pengumuman-aturan']));
    ...
    const handleGoogleLogin = useCallback(async (email, name, picture) => {
    ...
    const handleVerify = useCallback(async (email, code) => {
        if (verification && verification.email === email && verification.code === code) {
            const newUser = { email: verification.email, password: verification.pass, createdAt: Date.now() };
            setUsers(prev => ({ ...prev, [email]: newUser }));
            setCurrentUser(newUser);
    ...
    const handleIdCreated = useCallback((username, email) => {
        const userToUpdate = users[email];
        if (userToUpdate) {
            const updatedUser = { ...userToUpdate, username };
    ...
    const handleSelectCoin = useCallback(async (coinId) => {
        setIsTrendingLoading(true);
        setTrendingError(null);
        ...
        const fetchAndProcessNews = async () => {
            ...
            try {
                ...
                setMessages(prevMessages => {
                    ...
                    const existingNewsUrls = new Set(allMessagesInRoom
                        .filter((msg) => 'url' in msg)
                        .map(news => news.url));
    ...
    useEffect(() => {
        const simulationInterval = setInterval(() => {
            const joinedButNotActiveRooms = Array.from(joinedRoomIds).filter(id => id !== currentRoom?.id && !['berita-kripto', 'pengumuman-aturan'].includes(id));
    ...
    const handleIncrementAnalysisCount = useCallback((coinId) => {
    ...
    const handleLeaveJoinedRoom = useCallback((roomId) => {
    ...
    const renderActivePage = () => {
        switch (activePage) {
            case 'home':
                return _jsx(HomePage, { idrRate: idrRate, isRateLoading: isRateLoading, currency: currency, onIncrementAnalysisCount: handleIncrementAnalysisCount, fullCoinList: fullCoinList, isCoinListLoading: isCoinListLoading, coinListError: coinListError, heroCoin: heroCoin, otherTrendingCoins: otherTrendingCoins, isTrendingLoading: isTrendingLoading, trendingError: trendingError, onSelectCoin: handleSelectCoin, onReloadTrending: handleResetToTrending });
            case 'rooms':
                return _jsx(RoomsListPage, { rooms: rooms, onJoinRoom: handleJoinRoom, onCreateRoom: handleCreateRoom, totalUsers: totalUsers, hotCoin: hotCoin, userProfile: currentUser, currentRoomId: currentRoom?.id || null, joinedRoomIds: joinedRoomIds, onLeaveJoinedRoom: handleLeaveJoinedRoom, unreadCounts: unreadCounts });
            case 'forum':
                return _jsx(ForumPage, { room: currentRoom, messages: currentRoom ? messages[currentRoom.id] || [] : [], userProfile: currentUser, onSendMessage: handleSendMessage, onLeaveRoom: handleLeaveRoom, onReact: handleReaction });
            case 'about':
    ...
    if (!currentUser) {
        return _jsx(LoginPage, { onLogin: handleLogin, onRegister: handleRegister, onVerify: handleVerify, onGoogleLogin: handleGoogleLogin });
    }
    if (!currentUser.username) {
        return _jsx(CreateIdPage, { onIdCreated: handleIdCreated, email: currentUser.email });
    }
    return (_jsxs("div", { className: "min-h-screen bg-transparent text-white font-sans flex flex-col", children: [_jsx(Particles, {}), _jsx(Header, { userProfile: currentUser, onLogout: handleLogout, activePage: activePage, onNavigate: handleNavigate, currency: currency, onCurrencyChange: setCurrency, hotCoin: hotCoin, idrRate: idrRate }), _jsx("main", { className: "flex-grow", children: renderActivePage() }), _jsx(Footer, {})] }));
};
export default App;
